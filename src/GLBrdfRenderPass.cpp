...
void GLBrdfRenderPass::init(SceneGraph &scene)
{
    auto quad = createQuad();
    mVAO = GLDevice::createVAO(quad.vertices, quad.indices);
    mProgram = GLDevice::createProgram(brdfVS, brdfFS);

    mBrdfLut = GLDevice::createTexture(TextureDescriptor{
        .width = 512,
        .height = 512,
...
void GLBrdfRenderPass::render(SceneGraph &scene)
{
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, mBrdfLut.handle, 0);
    GLDevice::useProgram(mProgram);
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    GLDevice::drawVAO(mVAO);
